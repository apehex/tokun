...
def random_sample(sample_size: int, lower_plane: int=0, upper_plane: int=0x323af, binary: bool=False) -> list:
...
def random_dataset(sample_count: int, sample_size: int, lower_plane: int=0, upper_plane: int=0x323af, binary: bool=False, dtype: tf.dtypes.DType=tf.int32) -> tf.data.Dataset:
    ...
    return tf.data.Dataset.from_generator(
        ...
        output_signature=tf.TensorSpec(shape=(__factor * sample_size,), dtype=dtype))
