...
LINES = open('.data/shakespeare/othello.md', 'r').read() # .splitlines()
LINES += open('.data/shakespeare/hamlet.md', 'r').read() # .splitlines()
...
def dataset(words: list, stoi: callable=_stoi, context: int=N_CONTEXT, depth: int=N_VOCABULARY) -> tuple:
    __x = [_miv.encode(__n, stoi=stoi) for __w in words for __n in _min.tokenize(word=__w, length=context)]
    __y = [__i for __w in words for __i in _miv.encode(__w + '.', stoi=stoi)]
...
class Model(tf.Module):
    def __init__(self, n_context: int=N_CONTEXT, N_VOCABULARY: int=N_VOCABULARY, n_embedding: int=N_EMBEDDING, n_hidden: int=N_HIDDEN, *args, **kwargs):
        ...
        self._layers = [
            # embedding
            Embedding(input_dim=N_VOCABULARY, output_dim=n_embedding, name='embedding'),
            # block 1
            ...
            # head
            Dense(units=N_VOCABULARY, use_bias=True, name='head'),
...
X_TRAIN, Y_TRAIN = dataset(words=LINES[:N1], stoi=_stoi, context=N_CONTEXT)
X_DEV, Y_DEV = dataset(words=LINES[N1:N2], stoi=_stoi, context=N_CONTEXT)
X_TEST, Y_TEST = dataset(words=LINES[N2:], stoi=_stoi, context=N_CONTEXT)
