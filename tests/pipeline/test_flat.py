...
DATASET = tokun.data.random_dataset(sample_count=128, sample_size=256)
...
MODEL = tf.keras.models.load_model('models/vqvae.4.keras', compile=False)
...
class PreprocessTests(tf.test.TestCase):
    ...
    def test_on_dataset_batches(self):
        ...
        __preprocess = tokun.pipeline.flat.preprocess.factory(batch_dim=16, sample_dim=128, token_dim=4, drop_dim=0, encoding='UTF-8', features=['data'],)
        for _ in range(4):
            __s = next(__dataset)
            __x, __t = __preprocess(__s)
            # shapes
