...
class EncodeDecodeCodepointsTest(tf.test.TestCase):
    def setUp(self):
        ...
        self._encode = functools.partial(tokun.pipeline.encode, **self._config)
        # load the data
...
class EncodeDecodeBytesTest(tf.test.TestCase):
    def setUp(self):
        ...
        self._dataset_encoded = self._dataset_origin.map(self._encode)
        # decode back to the strings
        self._dataset_decoded = self._dataset_encoded.map(tokun.pipeline.codepoint)
        self._dataset_decoded = self._dataset_decoded.map(tokun.pipeline.decode)
...
class PreprocessTest(tf.test.TestCase):
    ...
    def test_shapes(self):
        __s0 = 'hello world'
        __x0 = tokun.pipeline.preprocess(text=__s0, token_size=16, output_dtype=tf.uint8, expand_dims=[])
        __x1 = tokun.pipeline.preprocess(text=__s0, token_size=16, output_dtype=tf.uint8, expand_dims=[1])
        __x2 = tokun.pipeline.preprocess(text=__s0, token_size=64, output_dtype=tf.int32, expand_dims=[])
    ...
    def test_padding(self):
        __s0 = 'hello world'
        __x0 = tokun.pipeline.preprocess(text=__s0, token_size=16, output_dtype=tf.uint8, expand_dims=[])
        __x1 = tokun.pipeline.preprocess(text=__s0, token_size=64, output_dtype=tf.int32, expand_dims=[])
        __p0 = (-len(__s0) % 4) * 4
        __p1 = (-len(__s0) % 16)
