...
class Model(tf.Module):
    def __init__(self, n_context: int=N_CONTEXT, n_encoding: int=N_ENCODING, n_embedding: int=N_EMBEDDING, n_hidden: int=N_HIDDEN, *args, **kwargs):
        ...
        # parameters
        self.parameters = [self._C, self._W1, self._W2, self._B1, self._B2] # [self._C, self._W1, self._W2, self._B1, self._B2, self._G1]

    def __call__(self, x):
        # embed the input vector / matrix
        __e = self.embed(x)
        # intermediate
        __i = self.intermediate(__e)
        # hidden layer
        __h = self.hidden(__i)
        # normalize
        ...
        # head layer
        return self.head(__h)

    def embed(self, x):
        return tf.reshape(x @ self._C, (x.shape[0], self._N_CONTEXT * self._N_EMBEDDING))

    def intermediate(self, e):
        return e @ self._W1 + self._B1

    def hidden(self, i):
        return tf.math.tanh(i)

    def head(self, h):
        return tf.nn.softmax(h @ self._W2 + self._B2)

# LOSS ########################################################################

def loss(target_y: tf.Tensor, predicted_y: tf.Tensor, regularization: float=0.):
    ...
    return __l(target_y, predicted_y) + regularization
...
def step(model: Model, x: tf.Tensor, y: tf.Tensor, rate: float=R_TRAINING) -> tuple:
    with tf.GradientTape() as __tape:
        ...
        # forward
        __e = model.embed(x)
        __i = model.intermediate(__e)
        __h = model.hidden(__i)
        __p = model.head(__h)
        # loss
        __reg = tf.math.reduce_mean(tf.math.square(1. - tf.math.reduce_std(__i, axis=0, keepdims=False)))
        __loss = loss(target_y=y, predicted_y=model(x), regularization=0.)
        # backward
    ...
    with summary.as_default():
        for __i, __ratios in enumerate(data):
            for __j, __r in enumerate(__ratios):
                tf.summary.scalar(model.parameters[__j].name + '_log10(gradient/value)', __r, step=__i + offset)
